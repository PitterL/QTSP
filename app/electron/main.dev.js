...
// Updi operation
const { NupdiWrapper } = require('../lib/nupdi/wrapper');
const updiinf = new NupdiWrapper();
console.log('<Main> Load Nupdi interface', updiinf);
updiinf.load();
...
ipcMain.on('openFile', (event, path) => {
  ...
  function readFile(path) {
    fs.readFile(path, 'utf-8', (err, data) => {
      ...
      event.sender.send('openFile-resp', { selectPath: path, data });
    });
  }
});
