...
import { makeSelectFilePath, makeSelectRelativeDir, makeSelectRegAddress, makeSelectRegConfig } from './selectors';
import { MapfileParser, HeaderFileParser, DataStructureParser} from './parser';
import { openFile, updateRegAddr, updateRegCfg } from  './actions';
...
export class MapParser extends React.Component<{}, MapParserState> {
  ...
  handleOpenFile() {
    ipcRenderer.once('openFile-resp', (event, {selectPath, data}) => {
      console.log(`<MapParser> openFile event:${event}, selectPath: ${selectPath}`); //return {data, path} obj
      ...
      if (filetype == 'map') {
        ...
        this.props.onUpdateRegAddr(parser.parse(data));
      }else if (filetype == 'h') {
        const parser = new HeaderFileParser();
        this.props.onUpdateRegCfg(parser.parse(data));
      }
  ...
  render() {
    ...
    console.log('<MapParser> props:', this.props);
